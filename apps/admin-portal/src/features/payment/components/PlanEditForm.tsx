...
import { useUpdatePlan } from '@musetrip360/payment-management/api';
import { Plan, PlanUpdate } from '@musetrip360/payment-management';

const planEditSchema = z.object({
  name: z.string().min(1, 'Tên gói là bắt buộc').max(100, 'Tên gói không được quá 100 ký tự'),
  description: z.string(),
  price: z.number().min(1000, 'Giá phải lớn hơn hoặc bằng 1000'),
  durationDays: z.number().min(1, 'Thời hạn phải ít nhất 1 ngày').max(3650, 'Thời hạn không được quá 10 năm'),
  maxEvents: z.number().min(0, 'Số sự kiện tối đa phải ít nhất 0'),
...
const PlanEditForm = ({ plan, open, onOpenChange, onSuccess }: PlanEditFormProps) => {
  ...
  const onSubmit = async (data: PlanEditFormData) => {
    if (!plan) return;

    setIsSubmitting(true);
    try {
      const planData: PlanUpdate = {
        id: plan.id,
        ...data,
        // Convert empty maxEvents to undefined for unlimited
        maxEvents: data.maxEvents === 0 ? undefined : data.maxEvents,
        discountPercent: data.discountPercent === 0 ? undefined : data.discountPercent,
      };
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Cập nhật gói đăng ký</DialogTitle>
          <DialogDescription>
            Cập nhật thông tin gói đăng ký "{plan.name}". Thay đổi các trường bên dưới.
          </DialogDescription>
        </DialogHeader>
          ...
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormDescription>0 = Không giới hạn</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
            ...
            <div className="flex justify-end space-x-2">
              ...
              <Button type="submit" disabled={isSubmitting}>
                {isSubmitting ? 'Đang cập nhật...' : 'Cập nhật gói'}
              </Button>
            </div>
          </form>
        </Form>
