/**
 * @fileoverview Search API Integration
 * Provides search functionality across different entity types
 */

import { useQuery, getHttpClient } from '@musetrip360/query-foundation';
...
export function useSearchSuggestions(query: string, enabled: boolean = true) {
  const cacheKey = createSearchCacheKey('suggestions', query);
  const minLength = 2;

  return useQuery<{ suggestions: string[] }>(
    cacheKey,
    () => getHttpClient().get<{ suggestions: string[] }>(`/api/v1/search/suggestions?q=${encodeURIComponent(query)}`),
    {
