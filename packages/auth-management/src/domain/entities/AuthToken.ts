export class AuthToken {
  ...
  static createAccessToken(
    ...
    return new AuthToken(value, 'access', userId, expiresAt, new Date(), scope, deviceId);
  }

  static createRefreshToken(value: string, userId: string, expiresInSeconds: number, deviceId?: string): AuthToken {
    const expiresAt = new Date(Date.now() + expiresInSeconds * 1000);
    return new AuthToken(value, 'refresh', userId, expiresAt, new Date(), [], deviceId);
  }
